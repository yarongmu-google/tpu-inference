...
class TestKernelSearchSpace(unittest.TestCase):
    ...
    def test_smoke_test_env_does_NOT_truncate_search_space(self):
        ...
        the runner (kernel/tune.run_kernel_tune stops at the first
        SUCCESS row). The search space itself is identical with or
        without SMOKE_TEST=1 — the truncation strategy ("first
        value of each axis") could pick infeasible combos."""
        import os as _os
        from unittest import mock as _m
        with _m.patch.dict(_os.environ, {"SMOKE_TEST": "1"}):
            space_smoke = kernel_search_space(
                self.dir, "x", max_num_seqs=128,
            )
        _os.environ.pop("SMOKE_TEST", None)
        space_full = kernel_search_space(
            self.dir, "x", max_num_seqs=128,
        )
