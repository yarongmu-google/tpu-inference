...
#
# Usage:
#   tools/benchmark/sweep.sh <spec.service> [extra sweep.py flags...]
#
...
#
# Any additional flags after <spec.service> are passed through to
...
usage() {
    cat <<EOF
Usage: $0 <spec.service> [extra sweep.py flags...]

  spec.service   Sweep spec JSON (e.g. tools/benchmark/sweeps/<name>.service).
EOF
    exit 1
}
