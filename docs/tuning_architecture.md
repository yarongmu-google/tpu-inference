...
- `.kernel` — projection of the most-recent `.kernel.raw/*.jsonl`. One winner per `tuning_key` (= per pin_key set: page_size, kernel_K, geometry).
...
```json
{
  ...
  "winners": {
    "throughput_max": {
      "combo":           {...env-vars...},
      "metrics":         {...},
      "kernel_pin_keys": {"page_size": 128, "kernel_K": 128, ...}
    },
    "ttft_min":       {"combo": {...}, "metrics": {...}, "kernel_pin_keys": {...}},
    "p99_min":        {"combo": {...}, "metrics": {...}, "kernel_pin_keys": {...}}
  }
}
```

`kernel_pin_keys` lives **inside each winner** (not at top level): different objectives can pick combos that ran against different kernel winners (e.g. throughput_max picked a high-mnss combo; ttft_min picked a low-mnss one). The deploy lookup reads `winners[objective].kernel_pin_keys` and resolves the matching `.kernel` row from there (§7).
...
→ grep `<workload>.kernel` files for `kernel_K: 128`, intersect with workloads whose `.service.winners.*.kernel_pin_keys.kernel_K == 128`.
...
When adding a new TPU generation (`v8x`?) or a new GPU family (`b200`?), create the `cases/<new_topo>/` subtree and the rest of the stack works unchanged. Three code sites name hardware identifiers today:

1. `core/discriminator.py` — `DEFAULT_HARDWARE = "tpu_v7x"` is the canonical default plus the `KNOWN_KERNEL_VARIANTS` gate that pairs hardware with plugin.
2. `core/accumulator._infer_topo_model` — workload-path parser; reads the topo segment of the path.
3. Per-plugin adapter selection in `run_kernel_tune` (and eventually a sibling at the service-side bench adapter).

A new hardware target updates (1) with the new identifier, may or may not touch (2) depending on path conventions, and adds an adapter at (3).
...
- **Tuning key (`tuning_key`)** — the kernel-side identifier of a problem instance: `(page_size, q_dtype, kv_dtype, num_q_heads, num_kv_heads, head_dim, max_model_len, sliding_window, case, kernel_K, code_revision)`. Two workloads with the same tuning_key share `.kernel` rows.
- **Tunable params (`tunable_params`)** — the kernel-side knobs being tuned: `(bq_sz, bkv_sz, bq_csz, bkv_csz, max_num_subseqs)`.
- **Pin keys** — a subset of `tuning_key` fields that the `.service` winner uses to identify which `.kernel` row applies. Today: `(page_size, kernel_K)`. Could expand if more axes start mattering.
