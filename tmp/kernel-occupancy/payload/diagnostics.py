...
@contextmanager
def phase(output: Path, name: str):
    path = output / 'phase-times.json'
    state = json.loads(path.read_text()) if path.exists() else {'phases': {}}
    state['active'] = name
    save(path=path, value=state)
    ...
    try:
        ...
        state['phases'][name] = {'seconds': seconds, 'status': status}
        state['active'] = None
        save(path=path, value=state)
        print(f'PHASE_FINISHED: {name}; {seconds:.3f}s; {status}', flush=True)
...
def probe(output: Path, *, collect_dumps: bool = True) -> bool:
