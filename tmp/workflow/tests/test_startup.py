...
class StartupTests(unittest.TestCase):
    ...
    def test_comparison_builder_failure_is_visible_without_submission(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            original = controller.ROOT.parent / 'serving-comparison'
            comparison = repository / 'tmp/serving-comparison'
            shutil.copytree(src=original, dst=comparison,
                            ignore=shutil.ignore_patterns('results', '__pycache__'))
            commands = repository / 'scripts/vllm/benchmarking'
            commands.mkdir(parents=True)
            (commands / 'bench_throughput_qwen_server.sh').write_text('# unused before submission\n')
