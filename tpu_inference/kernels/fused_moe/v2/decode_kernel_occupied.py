...
def _decode_moe_kernel(
    ...
    *kernel_refs, counts_vmem, counts_smem, counts_sem = refs
    if fp8:
        (tokens_local_hbm, route_in_hbm, w1_hbm, w2_hbm,
         ...
         b_w1s_x2_vmem, b_w2s_x2_vmem, recv_sem_amax) = kernel_refs
    else:
    ...
    def block_activity(slot):
        any_active, all_active = jnp.bool_(False), jnp.bool_(True)
        for expert in range(be):
            active = counts_smem[slot, 0, expert] > 0
            any_active = any_active | active
            all_active = all_active & active
        return any_active, all_active

    def fetch_sparse_weights(block, bw_sem_id, *, start: bool):
        # Skip whole experts; retain the original double-buffered weight slabs.
        # A single semaphore per tensor counts only descriptors actually issued.
        for expert in range(be):
            @pl.when(counts_smem[bw_sem_id, 0, expert] > 0)
            def copy_expert(expert=expert):
                for source, target, sem_index in (
                    (w1_hbm, b_w1_x2_vmem, 1),
                    (w2_hbm, b_w2_x2_vmem, 2),
                ):
                    copy = pltpu.make_async_copy(
                        src_ref=source.at[block * be + expert],
                        dst_ref=target.at[bw_sem_id, expert],
                        sem=local_sems.at[bw_sem_id, sem_index])
                    copy.start() if start else copy.wait()
        if fp8:
            # Scale rows retain their block-aligned DMA. They are small; unlike
            # the 3-D weights, their expert axis is a tiled matrix dimension.
            any_active, _ = block_activity(slot=bw_sem_id)
            @pl.when(any_active)
            def copy_scales():
                for source, target, sem_index in (
                    (w1s_hbm, b_w1s_x2_vmem, 1),
                    (w2s_hbm, b_w2s_x2_vmem, 2),
                ):
                    copy = pltpu.make_async_copy(
                        src_ref=source.at[pl.ds(block * be, be)],
                        dst_ref=target.at[bw_sem_id],
                        sem=local_sems.at[bw_sem_id, sem_index])
                    copy.start() if start else copy.wait()

    def start_fetch_bw(block, bw_sem_id):
        _, all_active = block_activity(slot=bw_sem_id)
        @pl.when(all_active)
        def dense():
            start_fetch_dense_bw(block=block, bw_sem_id=bw_sem_id)
        @pl.when(~all_active)
        def sparse():
            fetch_sparse_weights(block=block, bw_sem_id=bw_sem_id, start=True)

    def wait_fetch_bw(block, bw_sem_id):
        _, all_active = block_activity(slot=bw_sem_id)
        @pl.when(all_active)
        def dense():
            wait_fetch_dense_bw(bw_sem_id=bw_sem_id)
        @pl.when(~all_active)
        def sparse():
            fetch_sparse_weights(block=block, bw_sem_id=bw_sem_id, start=False)

    def start_fetch_dense_bw(block, bw_sem_id):
    ...
    def wait_fetch_dense_bw(bw_sem_id):
    ...
    def prepare_memberships(first_expert, slot):
        """Compute membership once and transfer one block's counts to SMEM.

        Metadata stays lane-major: [1, T] membership, [1, 128] counts.
        One 512-byte count transfer per expert block enables scalar guards.
        The count slot shares the existing x/weight parity lifetime.
        """
        if do_masks:
            tables = (idx_kt_vmem[...], w_kt_vmem[...])
            memberships = [dispatch_phase_a_valu(
                first_expert + expert, tables) for expert in range(be)]
            lanes = lax.broadcasted_iota(jnp.int32, (1, 128), 1)
            counts = jnp.zeros((1, 128), jnp.int32)
            for expert, (routed, _) in enumerate(memberships):
                counts += jnp.where(lanes == expert, jnp.sum(routed), 0)
        else:
            # Timing ablations keep their original full-capacity FFN work.
            memberships = []
            counts = jnp.full((1, 128), capacity, jnp.int32)
        counts_vmem.at[slot][...] = counts
        copy = pltpu.make_async_copy(
            src_ref=counts_vmem.at[slot], dst_ref=counts_smem.at[slot],
            sem=counts_sem)
        copy.start()
        copy.wait()
        return memberships

    def build_dispatch(memberships):
    ...
    def expert_gmm1_mxu(local_e_id: int, x_step, w1_step, *, row0: int, row_count: int):
        ...
        bd1c chunks the contraction. Returns [row_count, 2I] f32."""
        rows = slice(local_e_id * capacity + row0,
                     local_e_id * capacity + row0 + row_count)
        with jax.named_scope("moe_gmm1"):
            k_chunk = bd1c or hidden_size
            gate_up = jnp.zeros((row_count, w1_hbm.shape[-1]),
                                jnp.float32)                 # [row_count, 2I]
    ...
    def expert_scales_valu(local_e_id: int, gate_up, x_slot, *, row0: int):
        ...
        rows = pl.ds(local_e_id * capacity + row0, gate_up.shape[0])
    ...
    def expert_gmm2_mxu(local_e_id: int, act_out, y_step, w2_step,
        ...
        rows = slice(local_e_id * capacity + row0,
                     local_e_id * capacity + row0 + act_out.shape[0])
    ...
    # Grid step zero: routing -> metadata drain -> active weight fetch.
    # The token payload drains before the first dispatch gather.
    @pl.when(block_id == 0)
    def _prologue():
        # Inputs and barrier overlap. Weight fetch starts after routing
        # membership is known, so even block zero can skip empty experts.
        ...
        memberships = prepare_memberships(first_expert=0, slot=0)
        if do_weights:
            start_fetch_bw(block=0, bw_sem_id=0)
        # Always drain the remote sends, including the no-park ablation.
        if do_ag:
            wait_ag_tokens()
        if do_park:
            park_dispatch_mxu(
                build_dispatch(memberships) if do_masks else zero_pieces(),
                x_slot=0, ohg_slot=0)

    # Build next-block membership before its prefetch, reusing these values
    # in the original B/C dispatch-ahead pipeline below. The final phantom
    # block has zero counts and issues no weight DMA.
    next_memberships = prepare_memberships(
        first_expert=next_first_expert, slot=x_nxt)

    # ---- blocked weight fetch: start the NEXT block's slabs, then wait
    # THIS block's, both at the step top, before the guarded FFN.
    if do_weights:
        @pl.when(block_id + 1 < num_blocks)
        def _prefetch_weights():
            start_fetch_bw(block_id + 1, x_nxt)

        wait_fetch_bw(block=block_id, bw_sem_id=x_par)

    # Membership (A) moved before weight prefetch. Keep the original B/C
    # interleaving with this block's FFN; runtime row guards introduce new
    # scheduling boundaries whose cost must be measured against the control.
    if do_ffn:
        # The group combine reads every capacity row, even when its gate is
        # zero. Clear skipped rows so stale values or NaNs cannot propagate.
        y_vmem.at[group_pos][...] = jnp.zeros(
            (be * capacity, hidden_size), act_dtype)
    a_out, b_out, pieces = {}, {}, []
    for j in range(be + 2):
        if do_ffn and j < be:
            count = jnp.minimum(counts_smem[x_par, 0, j], capacity)
            first_rows = 32 if fp8 else 16
            for row0 in range(0, capacity, first_rows):
                @pl.when(row0 < count)
                def occupied_gmm1(row0=row0, expert=j):
                    gate_up = expert_gmm1_mxu(
                        local_e_id=expert, x_step=x_vmem.at[x_par],
                        w1_step=b_w1_x2_vmem.at[x_par],
                        row0=row0, row_count=first_rows)
                    if fp8 and do_scales:
                        gate_up = expert_scales_valu(
                            local_e_id=expert, gate_up=gate_up,
                            x_slot=x_par, row0=row0)
                    act_out = expert_act_eup(gate_up)
                    for offset in range(0, first_rows, 16):
                        @pl.when(row0 + offset < count)
                        def occupied_gmm2(offset=offset):
                            expert_gmm2_mxu(
                                local_e_id=expert,
                                act_out=act_out[offset:offset + 16, :],
                                y_step=y_vmem.at[group_pos],
                                w2_step=b_w2_x2_vmem.at[x_par],
                                s2_step=(b_w2s_x2_vmem.at[x_par]
                                         if (fp8 and do_scales) else None),
                                row0=row0 + offset)
        if do_masks:
            if j < be:
                a_out[j] = next_memberships[j]
            if 1 <= j <= be:
...
def fused_moe_decode_tp_fused(
    ...
    *,
    ...
    assert 1 <= be <= 128, be
    row_alignment = 32 if fp8 else 16
    assert capacity > 0 and capacity % row_alignment == 0, capacity
    if fp8:
    ...
    vmem_need = (
        2 * 8 * 128 * 4                       # count parity, f32-width native tiles
    ...
    scratch_shapes += [
        pltpu.VMEM((2, 1, 128), jnp.int32),   # one count lane per expert
        pltpu.SMEM((2, 1, 128), jnp.int32),   # 1 KiB scalar count parity
